...
- Typed API client abstraction with request timeout, normalized errors, retry policy, auth header injection, token refresh queuing, and sensitive-field log redaction.
...
- `src/store`: persisted Zustand stores for UI, auth, onboarding, application list, and new application draft state.
...
- Zustand is used for UI state, onboarding/auth state, persisted application data, and the new application draft.
...
- Full offline mutation queuing is not implemented. The architecture leaves room for an outbox store that can persist mutations and replay them when connectivity returns.
- Draft data is stored with AsyncStorage for Expo Go compatibility. In production, sensitive draft fields should be encrypted or moved to SecureStore/native encrypted storage.
...
- Add screen-level visual regression coverage for the five lifecycle states.
- Add secure local encryption for sensitive draft fields and stricter redaction coverage.
- Split long screen components further once real backend contracts are available.
